...
for value in "${values[@]}"; do
    ...
    command=(
        ...
        --gpu 2
        --output-dir "$REPO_ROOT/results/vary_num_annotations/jeff_splits/$station_annots_folder/pairs-train_$value"
        --model-dir "$REPO_ROOT/results/vary_num_annotations/jeff_splits/$station_annots_folder/pairs-train_$value/model"
        --augment --normalize
    )
