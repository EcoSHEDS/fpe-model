...
# example: Rscript rank-input.R -d /mnt/d/fpe/rank -s 29 -v FLOW_CFS
...
seed <- args$seed
images_hours <- c(args$min_hour, args$max_hour)
images_months <- c(args$min_month, args$max_month)
images_dates <- ymd(c(args$images_start, args$images_end))
annotations_dates <- ymd(c(args$annotations_start, args$annotations_end))
...
dataset_dir <- file.path(output_dir, station_id, "data")
log_info("dataset_dir: {dataset_dir}")
stopifnot(dir.exists(dataset_dir))

model_dir <- file.path(output_dir, station_id, "model")
...
if (!file.exists(file.path(dataset_dir, "annotations.csv"))) {
    log_info("annotations.csv not found, skipping")
...
log_info("exporting: {model_dir}")

subtitle <- glue("{station$name[[1]]} (ID={station_id})")
...
images_split %>%
  ...
  write_csv(file.path(model_dir, "images.csv"))
...
ggsave(file.path(model_dir, "images-ts.png"), plot = p, width = 8, height = 4)
...
ggsave(file.path(model_dir, "images-months.png"), plot = p, width = 6, height = 8)
...
ggsave(file.path(model_dir, "images-values-ts.png"), plot = p, width = 8, height = 6)
...
if (sum(!is.na(images_split$value)) > 0) {
  ...
  ggsave(file.path(model_dir, "images-values-cfd.png"), plot = p, width = 8, height = 6)
  ...
  ggsave(file.path(model_dir, "images-values-hist.png"), plot = p, width = 8, height = 6)
}

annotations %>%
  write_csv(file.path(model_dir, "annotations.csv"))

duplicate_pairs(pairs) %>%
  write_csv(file.path(model_dir, "pairs.csv"))
...
ggsave(file.path(model_dir, "pairs-timestamps.png"), p, width = 8, height = 6)
...
if (sum(!(is.na(pairs$value_1) & is.na(pairs$value_2))) > 0) {
  ...
  ggsave(file.path(model_dir, "pairs-values.png"), p, width = 8, height = 8)
  ...
  p <- p1 / p2
  ggsave(file.path(model_dir, "pairs-values-delta.png"), p, width = 8, height = 8)
}
...
write_file(manifest_json, file.path(model_dir, "manifest.json"))
write_lines(manifest, file.path(model_dir, "manifest.txt"))

file.copy(file.path(dataset_dir, "station.json"), file.path(model_dir, "station.json"))
...
list(
  ...
  write_json(file.path(model_dir, "rank-input.json"), auto_unbox = TRUE, pretty = TRUE)
