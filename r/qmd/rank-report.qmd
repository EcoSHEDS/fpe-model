...
dataset_dir <- file.path(site_dir, params$variable, params$dataset_version)
model_dir <- file.path(dataset_dir, "models", params$model_version)

station_file <- file.path(dataset_dir, "dataset", "station.json")

split_args_file <- file.path(model_dir, "input", "rank-input.json")
...
if (split_args$pairs$source == "oracle") {
...
pred <- pred %>% 
  ...
  mutate(
    ...
    z_obs = (log_value - mean(log_value, na.rm = TRUE)) / sd(log_value, na.rm = TRUE),
    z_pred = (score - mean(score, na.rm = TRUE)) / sd(score, na.rm = TRUE)
  )
...
Station:         `r stn$name`
Variable:        `r params$variable`
Dataset:         `r params$dataset_version`
Model:           `r params$model_version`
...
Annotations:
  Source:        `r split_args$pairs$source`
  # Train Pairs: `r split_args$pairs$splits$train`
  # Val Pairs:   `r split_args$pairs$splits$val`
  Period:        `r as_date(split_args$pairs$start)` to `r as_date(split_args$pairs$end)`
