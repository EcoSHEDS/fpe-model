...
def split_weekly_flow(x, test_size=0.2, seed=1):
    ...
    weeks["split"] = weeks["week_index"].map(
        lambda x: "train" if x in week_idx_train else "test"
    )
    df["split"] = df["week_index"].map(
        lambda x: "train" if x in week_idx_train else "test"
    )
...
def _create_datasets(train_df, test_df, data_dir):
    ...
    train_dataset = FlowPhotoDataset(
        train_df, data_dir, transform=transform, label_transform=label_transform
    )
    test_dataset = FlowPhotoDataset(
        test_df, data_dir, transform=transform, label_transform=label_transform
    )
...
def train(dataloader, model, loss_fn, optimizer, device):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        if batch % 1 == 0:
            print(
                f"train [batch {(batch + 1):>5d}/{num_batches:>5d}] loss: {loss.item():>7f}"
            )
...
def test(dataloader, model, loss_fn, device):
    ...
    with torch.no_grad():
        for batch, (X, y) in enumerate(dataloader):
            ...
            if batch % 1 == 0:
                print(
                    f"test [batch {(batch + 1):>5d}/{num_batches:>5d}] loss: {loss:>7f}"
                )
    avg_loss = total_loss / num_batches
...
if __name__ == "__main__":
    ...
    pred_dataset = FlowPhotoDataset(
        pred_df,
        args.data_dir,
        transform=test_dataset.transform,
        label_transform=test_dataset.label_transform,
    )
    pred_dataloader = torch.utils.data.DataLoader(
        pred_dataset, batch_size=args.test_batch_size, shuffle=False
    )
